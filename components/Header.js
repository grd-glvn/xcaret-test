...
function Header() {
    ...
    <div className="flex flex-wrap flex-col sm:flex-row xl:flex-row justify-center items-center">
      <h1 className='my-6 text-3xl uppercase sm:basis-full sm:text-5xl xl:text-7xl xl:basis-full xl:text-start'>{content.header.h1}</h1>
      {/* <div className='flex justify-center items-center'> */}
        <h2 className='text-3xl mb-6 sm:basis-1/2 sm:text-5xl xl:text-7xl sm:basis-1/4 text-center'>{content.header.discount}</h2>
        <div className="flex flex-wrap px-5 sm:basis-1/2 sm:basis-3/4 ">
          {content.header.paragraphs.map(paragraph => {
            return (
                <p className='text-md xl:text-xl text-gray-600'>{paragraph}</p>
          )
          })
          }
        </div>
