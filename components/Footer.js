...
function Footer() {
  ...
  return (
    ...
    <div className="flex gap-32 sm:gap-0 my-5">
      <a className="sm:mx-10 text-white text-xs order-2 sm:order-1" href={content.footer.links[0].href}>{content.footer.links[0].tittle}</a>
      <a className="text-white text-xs order-1 sm:order-2" href={content.footer.links[1].href}>{content.footer.links[1].tittle}</a>
    </div>
  </footer>
  )
}
